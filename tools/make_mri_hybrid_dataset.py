...
def make_train_examples(vol_name, seg_name, resample_size, patch_size, nb_samples_per_volume):
    ...
    image = np.pad(image, ((0, 0), (0, 0), (pad_len, pad_len)), mode='edge')
    label = np.pad(label, ((0, 0), (0, 0), (pad_len, pad_len)), mode='edge')

    examples = []
    
    for index in range(resample_size[2]):
        patch_image = image[:, :, index:index + patch_size]
        patch_label = label[:, :, index:index + patch_size]
        # pdb.set_trace()
        # if patch_label.max() == 0:
        #     continue
